...
// Instructor mirror of requireStudent: resolves the signed-in user to their
// directory row and gates on role === "instructor". Redirects to /login when
// there's no session / no row, and to /learn when a student lands on an
// instructor route (so the two apps stay separate without a hard error).
...
export async function getInstructorOrNull(): Promise<User | null> {
  const supabase = await createClient();
  if (!supabase) return null;
  ...
  return instructor?.role === "instructor" ? instructor : null;
}
